...
class PlotOffLine(object):
    def __init__(self, trace_file):
        ...
        self.line = Line(
            axes=self.ax,
            dic_plane=self.dic['J20'],
            dic_radar_1=self.dic['radars']['radar1']
        )
        self.suppress_fan = SuppressFan(
            axes=self.ax,
            dic_agents=self.dic["agents"],
            dic_radar_1=self.dic['radars']['radar1']
        )
        '''
        print(self.dic['radars']['radar1'])
        print(self.dic['radars']['radar2'])
        print(self.dic['J20'])
        '''

        A = ani.FuncAnimation(fig=self.fig, func=self.update, frames=int(10e10))
        import time
        #A.save(f"demo-{time.strftime('%Y-%m-%d-%H-%M-%S',time.localtime())}.gif", writer='pillow', dpi=200)
        plt.show()
    ...
    def load_data(self, trace_file):
        ...
        with open(trace_file) as fo:
            ...
            if 'J20' in self.dic:
                plane = J20(ax=self.ax, dic=self.dic['J20'])
                self.entity.append(plane)
    ...
    def update(self, itr):
        for entity in self.entity:
            ...
            #     # print(len(entity.X))
            entity.update(itr)
        self.line.update(itr)
        self.suppress_fan.update(itr)
