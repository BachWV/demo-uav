...
class ReplayBuffer(object):
    ...
    def save(self):
        ...
        print('save data success!!!', self.__len__())
    ...
    def add_train_data(self):
        datas = self.load()
        for tuple_data in datas:
            if self._next_idx >= len(self._storage):
                self._storage.append(tuple_data)
            else:
                self._storage[self._next_idx] = tuple_data
            self._next_idx = (self._next_idx + 1) % self._maxsize
