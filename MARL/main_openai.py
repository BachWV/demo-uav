...
def get_trainers(env, num_adversaries, obs_shape_n, action_shape_n, arglist):
    ...
    optimizer_c = [None for _ in range(env.n)]
    optimizer_a = [None for _ in range(env.n)]
    ...
    # if need to load old model, there should be a procedure for judging if the trainers[idx] is None
    for i in range(env.n):
        actors_cur = openai_actor(obs_shape_n[i], action_shape_n[i], arglist).to(arglist.device)
        critics_cur[i] = openai_critic(sum(obs_shape_n), sum(action_shape_n), arglist).to(arglist.device)
        actors_tar = openai_actor(obs_shape_n[i], action_shape_n[i], arglist).to(arglist.device)
        critics_tar[i] = openai_critic(sum(obs_shape_n), sum(action_shape_n), arglist).to(arglist.device)
        optimizer_a = optim.Adam(actors_cur[i].parameters(), arglist.lr_a)
        optimizer_c = optim.Adam(critics_cur[i].parameters(), arglist.lr_c)
...
def agents_train(arglist, game_step, update_cnt, memory, obs_size, action_size,
    ...
    if game_step > arglist.learning_start_step and \
        ...
        for agent_idx, (actor_c, actor_t, critic_c, critic_t, opt_a, opt_c) in enumerate(
            ...
            opt_c.zero_grad()
            loss_c.backward()
            nn.utils.clip_grad_norm(critic_c.parameters(), arglist.max_grad_norm)
            opt_c.step()

            # --use the data to update the ACTOR
            # There is no need to cal other agent's action
            model_out, policy_c_new = actor_c(_obs_n_o[:, obs_size[agent_idx][0]:obs_size[agent_idx][1]],
            ...
            nn.utils.clip_grad_norm(actor_c.parameters(), arglist.max_grad_norm)
            opt_a.step()
        # save the model to the path_dir ---cnt by update number
        if update_cnt > arglist.start_save_model and update_cnt % arglist.fre_save_model == 0:
            time_now = time.strftime('%y%m_%d%H%M')
            print('=time:{}step:{}     save'.format(time_now, game_step))
            model_file_dir = os.path.join(arglist.save_dir, '{}_{}'.format(time_now, game_step))
...
def train(arglist):
    ...
    env = make_env(arglist.scenerio_name, arglist, arglist.benchmark)
    print('=============================')
    ...
    num_adversaries = env.n
    actors_cur, critics_cur, actors_tar, critics_tar, optimizers_a, optimizers_c = get_trainers(env, num_adversaries,
                                                                                                obs_shape_n,
                                                                                                action_shape_n, arglist)
    ...
    for episode_gone in range(arglist.max_episode):
        ...
        for episode_cnt in range(arglist.per_episode_max_len):
            #get action
            action_n=[agent(torch.from_numpy(obs).to(arglist.device,torch.float)).detach().cpu().numpy() for agent,obs in zip(actors_cur,obs_n)]
            ...
            #train our agents
            update_cnt,actors_cur,actors_tar,critics_cur,critics_tar=agents_train(arglist,game_step,update_cnt,memory,obs_size,action_size,actors_cur,actors_tar,critics_cur,critics_tar,optimizers_a,optimizers_c)
